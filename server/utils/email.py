def message_template(client: str, lang: str, day: str, hour: str) -> str:
    """
    ...
    <p>Hello {client},</p>
     ...
     <p>We are writing to confirm the appointment you have made with us. The appointment will be on <strong>{day}</strong> at <strong>{hour}</strong>. Please RSVP by replying to this email.</p>
    ...
    """
        ...
        <p>Wir schreiben Ihnen, um den von Ihnen vereinbarten Termin zu bestätigen. Der Termin findet am <strong>{day}</strong> um <strong>{hour}</strong> statt. Bitte melden Sie sich an, indem Sie auf diese E-Mail antworten.</p>
    ...
    <p>Bonjour {client},</p>
     ...
     <p>Nous vous écrivons pour confirmer le rendez-vous que vous avez pris avec nous. Le rendez-vous aura lieu le <strong>{day}</strong> à <strong>{hour}</strong>. Veuillez confirmer votre présence en répondant à cet e-mail.</p>
    ...
    """
     ...
     <p>Ti scriviamo per confermare l'appuntamento che hai preso con noi. L'appuntamento sarà il <strong>{day}</strong> alle <strong>{hour}</strong>. Si prega di rispondere rispondendo a questa email.</p>
    ...
    <p>Olá {client},</p>
     ...
     <p>Estamos escrevendo para confirmar o agendamento que você fez conosco. O compromisso será em <strong>{day}</strong> às <strong>{hour}</strong>. Confirme presença respondendo a este e-mail.</p>
    ...
    """
     ...
     <p>私たちは、あなたとの約束を確認するために手紙を書いています。 予約は <strong>{day}</strong> の <strong>{hour}</strong> に行われます。 このメールに返信して出欠の旨をご連絡ください。</p>
    ...
    <p>Le escribimos para confirmar la cita que ha establecido con nosotros. La cita será el día <strong>{day}</strong> a las <strong>{hour}</strong>. Por favor, confirme su asistencia respondiendo a este correo electrónico.</p>
